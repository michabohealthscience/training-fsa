...
We can use a a few `for loops` in a `function` to calculate this for all features for a specific test substance.

Note there are multiple ways to acheive this, for loops are straightforward approach that do not require many additional packages however other functions and packages (like plyr and tidyr) can be used.
...
So lets run the function to get all the p values and fold changes 
```{r run_function}
...
```{r volcano_plot}
library(tidyr)
ttest_ts2 <- ttest_ts2[!is.na(ttest_ts2$pvalue),]
...
# Summary of t-tests
Rather than do this on all the test substances and all assays in this training we have a data file already created with the relevant t-stats that we can use for grouping in the next section.

Below though we show a summary of all the t-test analysis done across all assays and 

We note here that **multiple testing correction** was performed on the p-values to generate q-values. 

This was used as manual check to see if any of the test substance treatments were not exhibiting much bioactivity and a such should not be included in the grouping analysis.

It was deemed that for the male data, there was enough bioactivity to proceed to grouping without removing any test substance treatments. For the the female data however, treatment groups were removed (TSx)
